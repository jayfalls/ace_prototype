...
```
**Test cases (see `testing.md` §KeyBuilder Tests):**
- `TestNewKeyBuilder_EmptyNamespace` → `NewKeyBuilder("", "agent")` returns `ErrInvalidKey`
- `TestNewKeyBuilder_EmptyAgentID` → `NewKeyBuilder("ns", "")` returns `ErrAgentIDMissing`
- `TestBuild_ColonInAgentID` → `NewKeyBuilder("ns", "agent:bad")` returns `ErrInvalidKey`
- `TestBuild_FullKey` → `"ns:agent:et:id:v1", nil`
- `TestPattern_NoComponents` → `"ns:agent:*", nil`
- `TestPattern_EntityTypeOnly` → `"ns:agent:et:*", nil`
...
```
**Test cases (see `testing.md` §ValkeyBackend Tests):**
- `TestValkeyBackend_Get_Miss` → returns `ErrCacheMiss` for missing key
- `TestValkeyBackend_Get_Hit` → returns value after Set
- `TestValkeyBackend_Set_WithTTL` → TTL returns correct remaining duration
- `TestValkeyBackend_Delete_Exists` → subsequent Get returns `ErrCacheMiss`
- `TestValkeyBackend_GetMany_PartialHits` → returns only present keys
- `TestValkeyBackend_DeletePattern_BareStar` → returns `ErrPatternInvalid`
- `TestValkeyBackend_DeletePattern_ValidPattern` → succeeds
- `TestValkeyBackend_Close` → shuts down cleanly
...
```
**Test cases (see `testing.md` §SingleFlight Tests):**
- `TestSingleFlight_Do_100ConcurrentSameKey` → fetch function called exactly once
- `TestSingleFlight_Do_SecondCaller_Waits` → `shared=true` for all but the first caller
- `TestSingleFlight_Do_ErrorPropagation` → error propagated to all waiters
- `TestSingleFlight_DoChan_ResultOnChannel` → receives result on returned channel
- `TestSingleFlight_RaceCondition` → `go test -race` passes
...
```
**Test cases (see `testing.md` §Cache Core Tests):**
- `TestCache_Get_Miss` → `(nil, nil)` on empty cache
- `TestCache_Get_Hit` → returns value after Set
- `TestCache_Delete_Exists` then `TestCache_Get_Miss` → `(nil, nil)`
- `TestGetOrFetch_Miss_CallsFetch` → fetchFn called, value stored and returned
- `TestGetOrFetch_Hit_SkipsFetch` → returns cached value without calling fetchFn
- `TestGetOrFetch_StampedeProtection_Coalesced` → 100 concurrent calls, fetchFn called exactly once
- `TestWithNamespace_KeysIsolated` → keys resolved under different namespace
- `TestWithAgentID_KeysIsolated` → keys resolved under different agentID
- `TestCache_Get_NoAgentID` → `ErrAgentIDMissing`
- `TestCache_Set_ExceedsMaxSize` → `ErrMaxSizeExceeded`
- `TestCache_Set_NilValue` → error
...
```
**Test cases (see `testing.md` §Bulk Operations):**
- `TestGetMany_PartialHits` → 2 of 3 keys present → map has 2 entries
- `TestSetMany_Success` → stores all entries
- `TestDeleteMany_Success` → removes all specified keys
- `TestDeletePattern_BareStarRejected` → `ErrPatternInvalid`
- `TestDeletePattern_ValidPattern` → succeeds
- `TestDeleteByTag_TagExists` → removes all entries with that tag
...
```
**Test cases (see `testing.md` §Pattern & Tag Invalidation, §Versioned Invalidation):**
- `TestSet_WithTags` → tag index has entries
- `TestDeleteByTag_TagExists` → all entries with tag removed
- `TestInvalidateByVersion_Match` → key and `_version:{key}` deleted
- `TestInvalidateByVersion_Mismatch` → no-op (already stale)
- `TestInvalidateByVersion_KeyMissing` → no-op (already invalidated)
...
```
**Test cases (see `testing.md` §Observability):**
- `TestNoopObserver_NoPanic` → all operations complete without panic
- `TestObserver_AllOperationTypesCovered` → every observer method called for every operation type
- `TestStats_AfterOperations` → 10 hits and 5 misses → `HitCount=10`, `MissCount=5`, `HitRate≈0.667`
...
```
**Test cases (see `testing.md` §WarmingManager Tests):**
- `TestWarmOnStartup_CallsWarmFuncForEnabled` → WarmFunc called for each namespace with `OnStartup=true`
- `TestWarm_DeadlineExceeded` → `ErrWarmingTimeout`
- `TestTrackProgress_AfterWarm` → reflects entries populated during warming
- `TestWarm_FuncError` → error propagated to caller
- `TestWarm_ConcurrentNamespaces` → 3 namespaces warmed in parallel, no races
...
```
**Integration test cases (see `testing.md` §Integration Tests):**
- `TestIntegration_FullLifecycle` → full round-trip, no errors
- `TestIntegration_GetOrFetch_Coalescing` → 50 concurrent calls, fetchFn called once
- `TestIntegration_TagIndexLifecycle` → Set with tags → DeleteByTag → all tagged keys deleted
- `TestIntegration_BulkOperations` → SetMany → GetMany → DeleteMany → correct hit/miss
- `TestIntegration_NamespaceIsolation` → no cross-namespace access
- `TestIntegration_AgentIsolation` → no cross-agent access
