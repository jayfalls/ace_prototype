...
Sets up the documentation generation pipeline (`make test` target) before any artifacts are produced. This enables automated validation from the start.
...
**Verification**:
- `make test` runs without error (even if no docs exist yet)
...
- `make test` validates schema docs against live DB without discrepancies
...
- `make test` passes OpenAPI validation
...
- Mermaid syntax valid (verified by `mmdc` in `make test`)
...
1. **Makefile entry point**: All generation runs through `make test` (design/README.md)
...
4. **Validation mandatory**: `make test` must validate all generated artifacts (architecture.md §Validation Flow)
