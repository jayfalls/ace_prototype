...
val, err := cache.Get(ctx, "user:123")  // (nil, nil) on miss
cache.Delete(ctx, "user:123")

// Cache-aside with stampede protection
val, err = cache.GetOrFetch(ctx, "user:123", func(ctx context.Context) ([]byte, error) {
    return db.GetUser(ctx, "123")  // called once even with 50 concurrent callers
})

// Bulk invalidation by tag
cache.DeleteByTag(ctx, "user")  // deletes ALL keys tagged "user"

// Versioned invalidation
cache.Set(ctx, "config:v2", value, caching.WithVersion("2.0"))
cache.InvalidateByVersion(ctx, "config:v2", "2.0")  // no-op if version differs

// Stats
stats, _ := cache.Stats(ctx)  // HitCount, MissCount, HitRate
```

**Key format:** `{namespace}:{agentId}:{entityType}:{entityId}:{version}` — agentId is mandatory.
